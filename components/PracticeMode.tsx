...
const PracticeMode: React.FC<PracticeModeProps> = ({ verse, onClose, onStatusUpdate }) => {
  ...
  const renderVerseWithBlanks = () => {
      ...
      return (
        <p className="text-lg sm:text-xl md:text-2xl text-slate-800 font-serif leading-relaxed text-center whitespace-pre-line">
                ...
                <React.Fragment key={index}>
                    ...
                    {index < parts.length - 1 && (
                        <span 
                            ...
                            className={`inline-block bg-slate-200 rounded-md px-2 py-1 mx-1 text-center min-w-16 sm:min-w-24 cursor-pointer hover:bg-slate-300
  ...
  return (
    <div className="fixed inset-0 bg-slate-100 z-40 flex flex-col p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-4 sm:mb-6">
                <h2 className="text-lg sm:text-xl font-bold text-sky-700">{verse.reference} 연습</h2>
        ...
        <div className="flex-grow w-full max-w-4xl mx-auto flex flex-col justify-center items-center">
            <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-8 w-full">
                {renderVerseWithBlanks()}
            </div>
            
            <div className="mt-6 sm:mt-8 flex flex-wrap justify-center gap-2 sm:gap-3 w-full">
                {availableChoices.map((word, index) => (
                    <button
                        key={index}
                        onClick={() => handleWordBankClick(word)}
                        disabled={isSubmitting}
                        className="bg-white text-slate-700 font-semibold px-3 py-2 sm:px-4 sm:py-2.5 rounded-lg shadow-md border border-slate-300 hover:bg-sky-100 hover:border-sky-300 transition-all text-sm sm:text-base disabled:opacity-50"
                    >
                        {word}
                    </button>
                ))}
            ...
            {!isSubmitting ? (
                <div className="h-28 flex justify-center items-center">
                    <button 
                        ...
                        className="bg-green-600 text-white font-bold px-8 sm:px-12 py-3 sm:py-4 rounded-xl shadow-lg hover:bg-green-700 transition-transform hover:scale-105 text-base sm:text-lg disabled:bg-slate-300 disabled:cursor-not-allowed disabled:transform-none"
                    >
                        확인
                    </button>
                </div>
                ...
                <div className={`p-4 rounded-xl mt-6 text-white text-center transition-all duration-300 ${isCorrect ? 'bg-green-500' : 'bg-red-500'}`}>
                    <div className="flex items-center justify-center">
                        {isCorrect ? <CheckCircleIcon className="h-6 sm:h-8 w-6 sm:w-8 mr-3"/> : <XCircleIcon className="h-6 sm:h-8 w-6 sm:w-8 mr-3"/>}
                        <span className="text-base sm:text-lg font-bold">{isCorrect ? '정답입니다!' : '아쉽네요, 다시 시도해보세요!'}</span>
                    </div>
                     <div className="mt-4 flex flex-col sm:flex-row justify-center gap-2 sm:gap-3">
                        {isCorrect ? (
                             <>
                                <button onClick={fetchExercise} className="bg-white/20 hover:bg-white/40 text-white font-semibold py-2 px-4 rounded-lg text-xs sm:text-sm">
                                    다시 연습하기
                                </button>
                                <button onClick={() => onStatusUpdate(MemorizationStatus.InProgress)} className="bg-white/20 hover:bg-white/40 text-white font-semibold py-2 px-4 rounded-lg text-xs sm:text-sm">
                                    다음에 또 연습하기
                                </button>
                                <button onClick={() => onStatusUpdate(MemorizationStatus.Mastered)} className="bg-white/20 hover:bg-white/40 text-white font-semibold py-2 px-4 rounded-lg text-xs sm:text-sm">
                                    이제 완료했어요
                                </button>
                             </>
                        ): (
                             <button onClick={reset} className="bg-white/20 hover:bg-white/40 text-white font-semibold py-2 px-6 rounded-lg text-sm">
                                다시 시도
                             </button>
                        )}
